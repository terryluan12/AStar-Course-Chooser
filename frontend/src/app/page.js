...
function HomePage() {
  ...
  const handleSubmit = async (input, isLoggedIn) => {
    ...
    return searchCourse(input)
      .then((res) => {
        let results = [<Label key="empty"></Label>];
        results.push(
          res.data.courses.map((result) => {
            let isStarred = false;
            if (
              wishlist.some(
                (course) => course.course_code === result.course_code
              )
            ) {
              isStarred = true;
            }
  ...
  return <Search handler={handleSubmit} />;
}
